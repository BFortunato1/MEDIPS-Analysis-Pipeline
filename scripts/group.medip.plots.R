...
#	if("Batch" %in% colnames(met)) show.batch=T
	...
	met = subset(met, Class %in% c("case", "control", "reference_case", "reference_control"))
	...
	write.csv(df, file = "/n/scratch3/users/b/brf340/_RESTORE/FFPE_MeDIP/cfmedip/df.csv")
	...
	mtab <- data.frame(sample = met$SampleName, sparsity = sparsity, 
	  depth = depth,group = met$SampleName)                                         
	if(show.batch) mtab$batch = as.factor(met$Batch)                                              

	theme_set(theme_classic())
	if(show.batch) {
        mtab %>% ggplot(aes(x = sparsity, y = depth, color = met$SampleName, shape = batch)) +                                                                
          geom_point() 
	} else {                                                                                                               
        mtab %>% ggplot(aes(x = sparsity, y = depth, color = met$SampleName)) +                                                                
          geom_point() 
	}
	...
	if(show.batch) {
	ggplot(tidydf, aes(x=PC1, y=PC2, colour = met$SampleName, shape = batch)) + 
	  geom_point(size = 1) + 
	...
	ggplot(tidydf, aes(x=PC2, y=PC3, colour = met$SampleName, shape = batch)) + 
	  geom_point(size = 1) + 
	...
	ggplot(tidydf, aes(x=PC3, y=PC4, colour = met$SampleName, shape = batch)) +                              
	  geom_point(size = 1) +                                                        
	...
	ggsave(file.path(outdir, "PCA_3_4_cf_plasma.pdf"), width = 4, height = 3) 
        ...
        ggplot(tidydf, aes(x=PC1, y=PC2, colour = met$SampleName)) +
          geom_point(size = 1) +
	  xlab(paste0("PC1 (", varexp1, "% of variance explained)")) +
	  ylab(paste0("PC2 (", varexp2, "% of variance explained)"))
        ggsave(file.path(outdir, "PCA_1_2_cf_plasma.pdf"), width = 4, height = 3)

        ggplot(tidydf, aes(x=PC2, y=PC3, colour = met$SampleName)) +
          geom_point(size = 1) +
	  xlab(paste0("PC2 (", varexp2, "% of variance explained)")) +
	  ylab(paste0("PC3 (", varexp3, "% of variance explained)"))
        ggsave(file.path(outdir, "PCA_2_3_cf_plasma.pdf"), width = 4, height = 3)                                                                  
        ggplot(tidydf, aes(x=PC3, y=PC4, colour = met$SampleName)) + 
	   geom_point(size = 1) +
	   xlab(paste0("PC3 (", varexp3, "% of variance explained)")) +
	   ylab(paste0("PC4 (", varexp4, "% of variance explained)"))
        ggsave(file.path(outdir, "PCA_3_4_cf_plasma.pdf"), width = 4, height = 3) 
	...
	ggplot() +
	  ...
	  geom_text(data=text.df, aes(x=x, y=y, label=label, hjust=0,color=met$SampleName),size=2) +
